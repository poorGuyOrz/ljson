...
https://www.geeksforgeeks.org/b-tree-set-1-introduction-2/ for previous article.
...
void BTreeNode::insertNonFull(int k) {
  ...
  } else  // If this node is not leaf
  {
    ...
    // 如果child满了，进行分裂操作，由于此方法在调用的时候已经确保当前
    // 节点一定不是满的。所以分列不会向上传递
    if (C[i + 1]->n == 2 * t - 1) {
...
void BTreeNode::splitChild(int i, BTreeNode *y) {
  ...
  //  当前节点多了一个child，需要调整位置。包括child的指针，以及元素
...
int main() {
  BTree t(2);  // 使用树的度初始化B树

  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<> dis(1, 200);

  for (int i = 0; i < 20; i++) t.insert(dis(gen));
